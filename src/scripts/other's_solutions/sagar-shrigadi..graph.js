...
export function knightMoves2(start, end){
  let queue = [[start]];
  let visited = new Set();
  visited.add(start.toString());  // dilvik addition

  let i = 0;  // dilvik addition
  while (i < queue.length) {  // dilvik change
    let currentPath = queue[i];
    let currentPosition = currentPath[currentPath.length - 1];

    if (currentPosition[0] === end[0] && currentPosition[1] === end[1]){
      return currentPath;  // dilvik change
    }
